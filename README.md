...
On the contract dashboard, the weekly table does not show activity types. To calculate client contact progress, the script scans the linked week logbooks in hidden same-site iframes and caches the `Client Contact` hours locally. The client contact forecast uses the weekly logbook summary `Direct client total` where available, then uses the scanned total and the number of scanned weeks that contain activity to project that average across the remaining placement weeks.
